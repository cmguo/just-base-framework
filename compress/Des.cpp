...
#include "framework/compress/Des.h"
...
namespace framework
{
    namespace security
    {
        namespace des
        {
            ...
            int des3_enc(unsigned char *key,int keyLen,unsigned char *data,unsigned char *sCipher)
            {
                des_ctx dc;
                ...
                {
                    des_key(&dc,key);
                    memcpy(tmpData1,data,8);
                    des_enc(&dc,tmpData1,1);
                    des_key(&dc,key+8);
                    des_dec(&dc,tmpData1,1);
                    des_key(&dc,key);
                    des_enc(&dc,tmpData1,1);
                    memcpy(sCipher,tmpData1,8);		
                }
                else if(keyLen == 24 )
                {
                    des_key(&dc,key);
            ...
            int des3_dec(unsigned char *key,int keyLen,unsigned char *sCipher,unsigned char *sPlain)
            {
                des_ctx dc;
                unsigned char tmpData1[256];
                if(keyLen == 16)
                {
                    des_key(&dc,key);
                    memcpy(tmpData1,sCipher,8);
                    des_dec(&dc,tmpData1,1);
                    des_key(&dc,key+8);
                    des_enc(&dc,tmpData1,1);
                    des_key(&dc,key);
                    des_dec(&dc,tmpData1,1);
                    memcpy(sPlain,tmpData1,8);
                }
                else if(keyLen == 24 )
                {
                    des_key(&dc,key+16);
            ...
            int Des3EnCrypt(unsigned char *key,int keyLen,unsigned char *sData,int iDataLen,unsigned char *sCipher,int *iCipherLen)
            {
                ...
                blocks=iDataLen/8+1;
                ps=8-iDataLen%8;
                memcpy(tmpData,sData,iDataLen);
                memset(tmpData+iDataLen,ps,ps);
                for(int i=0;i<blocks;i++)
                {
                    des3_enc(key,keyLen,tmpData+i*8,sCipher+i*8);
                }
                *iCipherLen = 8*blocks;
                return 0;
            }
            ...
            int Des3DeCrypt(unsigned char *key,int keyLen,unsigned char *sCipher,int iCipherLen,unsigned char *sPlain,int *iPlainLen)
            {
                ...
                for(int i=0;i<blocks;i++)
                {
                    des3_dec(key,keyLen,sCipher+i*8,tmpData+8*i);
                }
                ps=tmpData[iCipherLen-1];
                memcpy(sPlain,tmpData,iCipherLen-ps);
                *iPlainLen = iCipherLen-ps;
                return 0;
            }
